...
### 本地优先
所有数据存储在本地 Room 数据库，不上传云端。语音识别使用设备端 Whisper，无需网络。
...
├── di/             # Hilt 依赖注入模块
└── util/           # 工具类 (ApiKeyManager, Result, 等)
```

## 语音输入

语音识别已预留 UI 入口 (DetailScreen 中的麦克风按钮)，但 Whisper 集成待实现。目前 `isRecording` 状态仅切换图标，不执行实际录音。
